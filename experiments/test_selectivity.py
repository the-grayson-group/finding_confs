...
def run_sequential_opt(features, dft_energies, init_sampler, enant):
	...
	while len(unseen_indices) > 0:
		...
		acq_func.process_sample(acq_scores, dft_energies, seen_indices,
			unseen_indices)
		n_samples += 1
	print("%s-Samples = %d" % (enant, n_samples))
	print("%s-Proportion = %.3f" % (enant, n_samples / features.shape[0]))
...
def run_experiment(ff_sdf_files, ff_energy_files, dft_energy_files):
	...
	ff_energy_files, dft_energy_files):
		...
		for enant, indices in enant_indices.items():
			init_sampler = ForceFieldSampler(ff_energies[indices])
			enant_features = features[indices]
			energies = dft_energies[indices]
			run_sequential_opt(enant_features, energies, init_sampler, enant)

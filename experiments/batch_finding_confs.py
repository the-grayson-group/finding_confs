...
from acquisition_functions import BatchedPI, KrigingBelieverPI, MinConstantLiarPI, MaxConstantLiarPI, MeanConstantLiarPI
...
def convert_expts_to_objs(acq, init):
	acq_funcs = (BatchedPI, KrigingBelieverPI, MinConstantLiarPI, MaxConstantLiarPI, MeanConstantLiarPI)
	acq_func = acq_funcs[acq]
	samplers = (ForceFieldSampler,)
	sampler = samplers[init]
	return acq_func, sampler

def minimise_conf_energy(x, y, seen_indices, unseen_indices, model, acq_func, n_samples):
	...
	while len(unseen_indices) > 0:
		...
		print(f"{n_samples} {min_y:.5f} {last_y:.5f}")
		n_samples += acq_func.select_next(x, y, seen_indices, unseen_indices, model, 5, True)
	min_y = 627.509 * (np.min(y[seen_indices]) - target_y)
	last_y = 627.509 * (y[seen_indices[-1]] - target_y)
	print(f"{n_samples} {min_y:.5f} {last_y:.5f}")

def run_seeded_experiment(expt, features, dft_energies, ff_energies, seed):
	acq, init, ff_treat, dims = expt
	print(f"# {acq} {init} {ff_treat} {dims}")
	...
	if len_scale < 0.0:
		print("ERROR: Failed to calculate bandwidth.")
		exit(1)
	...
	minimise_conf_energy(features, dft_energies, seen_indices, unseen_indices, model, acq_func, n_samples)

def run_experiments(feature_file, dft_file, ff_file, n_repeats):
	...
	for expt in experiments:
		for seed in range(1, n_repeats + 1):
			np.random.seed(seed)
			print(f"# {molecule_name} {seed}")
			run_seeded_experiment(expt, features, dft_energies, ff_energies, seed)
...
if __name__ == "__main__":
	...
	for i in range(len(feature_files)):
		run_experiments(feature_files[i], dft_files[i], ff_files[i], n_repeats)
